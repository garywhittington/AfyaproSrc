...
{
    public class clsRCHVaccines : MarshalByRefObject
    {
        #region declaration

        private static String pClassName = "AfyaPro_MT.clsRCHVaccines";
        ...
        public DataTable View(String mFilter, String mOrder, string mLanguageName, string mGridName)
        {
            ...
            try
            {
                string mCommandText = "select * from rch_vaccines";
                ...
                DataTable mDataTable = new DataTable("rch_vaccines");
            ...
            string mUserId)
        {
            ...
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.rchvaccines_add.ToString(), mUserId);
            if (mGranted == false)
            {
            ...
            try
            {
                mCommand.CommandText = "select * from rch_vaccines where code='"
                + mCode.Trim() + "'";
                mDataReader = mCommand.ExecuteReader();
                if (mDataReader.Read() == true)
                {
                    mResult.Exe_Result = 0;
                    mResult.Exe_Message = "Code is already in use by another vaccine";
                    return mResult;
                }
            }
            ...
            try
            {
                ...
                mDataFields.Add(new clsDataField("displayorder", DataTypes.dbnumber.ToString(), mDisplayOrder));
                mDataFields.Add(new clsDataField("inactive", DataTypes.dbnumber.ToString(), mInactive));

                mCommand.CommandText = clsGlobal.Get_InsertStatement("rch_vaccines", mDataFields);
            ...
            string mUserId)
        {
            ...
            #region check security for this function
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.rchvaccines_edit.ToString(), mUserId);
            if (mGranted == false)
            {
                mResult.Exe_Result = 0;
                mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.GRL_AccessDeniedToSystemFunction.ToString();
            ...
            try
            {
                mCommand.CommandText = "select * from rch_vaccines where code='" + mCode.Trim() + "'";
                mDataReader = mCommand.ExecuteReader();
                if (mDataReader.Read() == false)
                {
                    mResult.Exe_Result = 0;
                    mResult.Exe_Message = "Vaccine code does not exist";
                    return mResult;
                }
            }
            catch (OdbcException ex)
            ...
            try
            {
                ...
                mCommand.CommandText = clsGlobal.Get_UpdateStatement("rch_vaccines", mDataFields, "code='" + mCode.Trim() + "'");
                mCommand.ExecuteNonQuery();
        ...
        public AfyaPro_Types.clsResult Delete(String mCode, string mUserId)
        {
            ...
            #region check security for this function
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.rchvaccines_delete.ToString(), mUserId);
            if (mGranted == false)
            {
                mResult.Exe_Result = 0;
                mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.GRL_AccessDeniedToSystemFunction.ToString();
            ...
            try
            {
                ...
                //rch_vaccines
                mCommand.CommandText = clsGlobal.Get_DeleteStatement("rch_vaccines", "code='" + mCode.Trim() + "'");
                mCommand.ExecuteNonQuery();
