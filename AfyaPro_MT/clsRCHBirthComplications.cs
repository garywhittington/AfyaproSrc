...
{
    public class clsRCHBirthComplications : MarshalByRefObject
    {
        #region declaration

        private static String pClassName = "AfyaPro_MT.clsRCHBirthComplications";
        ...
        public DataTable View(String mFilter, String mOrder, string mLanguageName, string mGridName)
        {
            ...
            try
            {
                string mCommandText = "select * from rch_birthcomplications";
                ...
                DataTable mDataTable = new DataTable("rch_birthcomplications");
            ...
            string mUserId)
        {
            ...
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.rchbirthcomplications_add.ToString(), mUserId);
            if (mGranted == false)
            {
            ...
            try
            {
                mCommand.CommandText = "select * from rch_birthcomplications where code='"
                + mCode.Trim() + "'";
                mDataReader = mCommand.ExecuteReader();
                if (mDataReader.Read() == true)
                {
                    mResult.Exe_Result = 0;
                    mResult.Exe_Message = "Code is already in use by another complication";
                    return mResult;
                }
            }
            ...
            try
            {
                ...
                mCommand.CommandText = clsGlobal.Get_InsertStatement("rch_birthcomplications", mDataFields);
            ...
            string mUserId)
        {
            ...
            #region check security for this function
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.rchbirthcomplications_edit.ToString(), mUserId);
            if (mGranted == false)
            {
                mResult.Exe_Result = 0;
                mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.GRL_AccessDeniedToSystemFunction.ToString();
            ...
            try
            {
                mCommand.CommandText = "select * from rch_birthcomplications where code='" + mCode.Trim() + "'";
                mDataReader = mCommand.ExecuteReader();
                if (mDataReader.Read() == false)
                {
                    mResult.Exe_Result = 0;
                    mResult.Exe_Message = "Complication code does not exist";
                    return mResult;
                }
            }
            catch (OdbcException ex)
            ...
            try
            {
                ...
                mCommand.CommandText = clsGlobal.Get_UpdateStatement("rch_birthcomplications", mDataFields, "code='" + mCode.Trim() + "'");
                mCommand.ExecuteNonQuery();
        ...
        public AfyaPro_Types.clsResult Delete(String mCode, string mUserId)
        {
            ...
            #region check security for this function
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.rchbirthcomplications_delete.ToString(), mUserId);
            if (mGranted == false)
            {
                mResult.Exe_Result = 0;
                mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.GRL_AccessDeniedToSystemFunction.ToString();
            ...
            try
            {
                ...
                //rch_birthcomplications
                mCommand.CommandText = clsGlobal.Get_DeleteStatement("rch_birthcomplications", "code='" + mCode.Trim() + "'");
                mCommand.ExecuteNonQuery();
