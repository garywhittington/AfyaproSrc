...
{
    public class clsBillItemSubGroups : MarshalByRefObject
    {
        #region declaration

        private static String pClassName = "AfyaPro_MT.clsBillItemSubGroups";
        ...
        public DataTable View(String mFilter, String mOrder, string mLanguageName, string mGridName)
        {
            ...
            try
            {
                string mCommandText = "select * from facilitybillingsubgroups";
                ...
                DataTable mDataTable = new DataTable("facilitybillingsubgroups");
                ...
                foreach (DataRow mDataRow in mDataTable.Rows)
                {
                    ...
                    if (mRowIndex >= 0)
                    {
                        mCountryDescription = mDvGroups[mRowIndex]["description"].ToString().Trim();
                    }

                    mDataRow["groupdescription"] = mCountryDescription;
                    mDataRow.EndEdit();
        ...
        public AfyaPro_Types.clsResult Add(Int16 mGenerateCode, String mCode, String mDescription,
            String mGroupCode, string mUserId)
        {
            ...
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.blsitemsubgroups_add.ToString(), mUserId);
            if (mGranted == false)
            {
            ...
            if (mGenerateCode == 1)
            {
                ...
                mObjCode = objAutoCodes.Next_Code(
                    Convert.ToInt16(AfyaPro_Types.clsEnums.SystemGeneratedCodes.billingitemsubgroupcode), "facilitybillingsubgroups", "code");
            ...
            try
            {
                mCommand.CommandText = "select * from facilitybillingsubgroups where code='"
                + mCode.Trim() + "'";
                mDataReader = mCommand.ExecuteReader();
                if (mDataReader.Read() == true)
                {
                    mResult.Exe_Result = 0;
                    mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.BLS_BillingItemSubGroupCodeIsInUse.ToString();
                    return mResult;
                }
            }
            ...
            try
            {
                ...
                //execute statements
                mCommand.CommandText = "insert into facilitybillingsubgroups(code,description,groupcode) values('"
                + mCode.Trim() + "','" + mDescription.Trim() + "','" + mGroupCode.Trim() + "')";
        ...
        public AfyaPro_Types.clsResult Edit(String mCode, String mDescription, string mUserId)
        {
            ...
            #region check security for this function
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.blsitemsubgroups_edit.ToString(), mUserId);
            if (mGranted == false)
            {
                mResult.Exe_Result = 0;
                mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.GRL_AccessDeniedToSystemFunction.ToString();
            ...
            try
            {
                mCommand.CommandText = "select * from facilitybillingsubgroups where code='" + mCode.Trim() + "'";
                mDataReader = mCommand.ExecuteReader();
                if (mDataReader.Read() == false)
                {
                    mResult.Exe_Result = 0;
                    mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.BLS_BillingItemSubGroupCodeDoesNotExist.ToString();
                    return mResult;
                }
            }
            catch (OdbcException ex)
            ...
            try
            {
                ...
                mCommand.CommandText = "update facilitybillingsubgroups set description = '"
        ...
        public AfyaPro_Types.clsResult Delete(String mCode, string mUserId)
        {
            ...
            #region check security for this function
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.blsitemsubgroups_delete.ToString(), mUserId);
            if (mGranted == false)
            {
                mResult.Exe_Result = 0;
                mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.GRL_AccessDeniedToSystemFunction.ToString();
            ...
            #region check if sub-group is in use billing items
            try
            {
                mCommand.CommandText = "select * from facilitybillingitems where subgroupcode='" + mCode.Trim() + "'";
                mDataReader = mCommand.ExecuteReader();
                if (mDataReader.Read() == true)
                {
                    mResult.Exe_Result = 0;
            ...
            try
            {
                ...
                mCommand.CommandText = "delete from facilitybillingsubgroups where code='" + mCode.Trim() + "'";
                mCommand.ExecuteNonQuery();
