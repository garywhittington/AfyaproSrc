...
{
    public class clsRCHDangerIndicators : MarshalByRefObject
    {
        #region declaration

        private static String pClassName = "AfyaPro_MT.clsRCHDangerIndicators";
        ...
        public DataTable View(String mFilter, String mOrder, string mLanguageName, string mGridName)
        {
            ...
            try
            {
                string mCommandText = "select * from rch_dangerindicators";
                ...
                DataTable mDataTable = new DataTable("rch_dangerindicators");
            ...
            string mUserId)
        {
            ...
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.rchdangerindicators_add.ToString(), mUserId);
            if (mGranted == false)
            {
            ...
            try
            {
                mCommand.CommandText = "select * from rch_dangerindicators where code='"
                + mCode.Trim() + "'";
                mDataReader = mCommand.ExecuteReader();
                if (mDataReader.Read() == true)
                {
                    mResult.Exe_Result = 0;
                    mResult.Exe_Message = "Code is already in use by another indicator";
                    return mResult;
                }
            }
            ...
            try
            {
                ...
                mDataFields.Add(new clsDataField("quantityentry", DataTypes.dbnumber.ToString(), mQuantityEntry));
                mDataFields.Add(new clsDataField("inactive", DataTypes.dbnumber.ToString(), mInactive));

                mCommand.CommandText = clsGlobal.Get_InsertStatement("rch_dangerindicators", mDataFields);
            ...
            string mUserId)
        {
            ...
            #region check security for this function
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.rchdangerindicators_edit.ToString(), mUserId);
            if (mGranted == false)
            {
                mResult.Exe_Result = 0;
                mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.GRL_AccessDeniedToSystemFunction.ToString();
            ...
            try
            {
                mCommand.CommandText = "select * from rch_dangerindicators where code='" + mCode.Trim() + "'";
                mDataReader = mCommand.ExecuteReader();
                if (mDataReader.Read() == false)
                {
                    mResult.Exe_Result = 0;
                    mResult.Exe_Message = "Indicator code does not exist";
                    return mResult;
                }
            }
            catch (OdbcException ex)
            ...
            try
            {
                ...
                mCommand.CommandText = clsGlobal.Get_UpdateStatement("rch_dangerindicators", mDataFields, "code='" + mCode.Trim() + "'");
                mCommand.ExecuteNonQuery();
        ...
        public AfyaPro_Types.clsResult Delete(String mCode, string mUserId)
        {
            ...
            #region check security for this function
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.rchdangerindicators_delete.ToString(), mUserId);
            if (mGranted == false)
            {
                mResult.Exe_Result = 0;
                mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.GRL_AccessDeniedToSystemFunction.ToString();
            ...
            try
            {
                ...
                //rch_dangerindicators
                mCommand.CommandText = clsGlobal.Get_DeleteStatement("rch_dangerindicators", "code='" + mCode.Trim() + "'");
                mCommand.ExecuteNonQuery();
