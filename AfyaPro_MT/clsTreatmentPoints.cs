...
{
    public class clsTreatmentPoints : MarshalByRefObject
    {
        #region declaration

        private static String pClassName = "AfyaPro_MT.clsTreatmentPoints";
        ...
        public DataTable View(String mFilter, String mOrder, string mLanguageName, string mGridName)
        {
            ...
            try
            {
                string mCommandText = "select * from facilitytreatmentpoints";
                ...
                DataTable mDataTable = new DataTable("facilitytreatmentpoints");
        ...
        #region View_ConsultationCount
        public DataTable View_ConsultationCount()
        {
            String mFunctionName = "View_ConsultationCount";
            ...
            try
            {
                mCommand.CommandText = "select treatmentpointcode,count(patientcode) patientscount from patientsqueue"
                    + " where queuetype='consultation' group by treatmentpointcode";

                DataTable mDataTable = new DataTable("patientsqueue");
                mDataTable.RemotingFormat = SerializationFormat.Binary;
                mDataAdapter.SelectCommand = mCommand;
                mDataAdapter.Fill(mDataTable);
        ...
        public AfyaPro_Types.clsResult Add(Int16 mGenerateCode, String mCode, String mDescription, string mUserId)
        {
            ...
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.gentreatmentpoints_add.ToString(), mUserId);
            if (mGranted == false)
            {
            ...
            if (mGenerateCode == 1)
            {
                ...
                mObjCode = objAutoCodes.Next_Code(
                    Convert.ToInt16(AfyaPro_Types.clsEnums.SystemGeneratedCodes.treatmentpointcode), "facilitytreatmentpoints", "code");
            ...
            try
            {
                mCommand.CommandText = "select * from facilitytreatmentpoints where code='"
                + mCode.Trim() + "'";
                mDataReader = mCommand.ExecuteReader();
                if (mDataReader.Read() == true)
                {
                    mResult.Exe_Result = 0;
                    mResult.Exe_Message = "Treatment point code is in use";
                    return mResult;
                }
            }
            ...
            try
            {
                ...
                if (mGenerateCode == 1)
                {
                    ...
                    + Convert.ToInt16(AfyaPro_Types.clsEnums.SystemGeneratedCodes.treatmentpointcode);
                    mCommand.ExecuteNonQuery();
                }
        ...
        public AfyaPro_Types.clsResult Edit(String mCode, String mDescription, string mUserId)
        {
            ...
            #region check security for this function
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.gentreatmentpoints_edit.ToString(), mUserId);
            if (mGranted == false)
            {
                mResult.Exe_Result = 0;
                mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.GRL_AccessDeniedToSystemFunction.ToString();
            ...
            try
            {
                mCommand.CommandText = "select * from facilitytreatmentpoints where code='" + mCode.Trim() + "'";
                mDataReader = mCommand.ExecuteReader();
                if (mDataReader.Read() == false)
                {
                    mResult.Exe_Result = 0;
                    mResult.Exe_Message = "Treatment point code does not exist";
                    return mResult;
                }
            }
            catch (OdbcException ex)
            ...
            try
            {
                ...
                mCommand.CommandText = "update facilitytreatmentpoints set description = '"
        ...
        public AfyaPro_Types.clsResult Delete(String mCode, string mUserId)
        {
            ...
            #region check security for this function
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.gentreatmentpoints_delete.ToString(), mUserId);
            if (mGranted == false)
            {
                mResult.Exe_Result = 0;
                mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.GRL_AccessDeniedToSystemFunction.ToString();
            ...
            try
            {
                ...
                mCommand.CommandText = "delete from facilitytreatmentpoints where code='" + mCode.Trim() + "'";
                mCommand.ExecuteNonQuery();
