...
{
    public class clsSomCustomers : MarshalByRefObject
    {
        #region declaration

        private static String pClassName = "AfyaPro_MT.clsSomCustomers";
        ...
        public DataTable View(String mFilter, String mOrder, string mLanguageName, string mGridName)
        {
            ...
            try
            {
                string mCommandText = "select * from som_customers";
                ...
                DataTable mDataTable = new DataTable("som_customers");
        ...
        public AfyaPro_Types.clsResult Add(Int16 mGenerateCode, String mCode, String mDescription,
            string mPhone, string mAddress, string mFax, string mEmail, string mWebsite,
            DataTable mDtStores, string mUserId)
        {
            ...
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.ivscustomers_add.ToString(), mUserId);
            if (mGranted == false)
            {
            ...
            if (mGenerateCode == 1)
            {
                ...
                mObjCode = objAutoCodes.Next_Code(
                    Convert.ToInt16(AfyaPro_Types.clsEnums.SystemGeneratedCodes.customerid), "som_customers", "code");
            ...
            try
            {
                mCommand.CommandText = "select * from som_customers where code='"
                + mCode.Trim() + "'";
                mDataReader = mCommand.ExecuteReader();
                if (mDataReader.Read() == true)
                {
                    mResult.Exe_Result = 0;
                    mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.IVS_CustomerCodeIsInUse.ToString();
                    return mResult;
                }
            }
            ...
            try
            {
                ...
                //som_customers
                mCommand.CommandText = "insert into som_customers(code,description,phone,address,"
                + "fax,email,website) values('" + mCode.Trim() + "','" + mDescription.Trim()
                + "','" + mPhone + "','" + mAddress + "','" + mFax + "','" + mEmail + "','" + mWebsite + "')";
                ...
                foreach (DataRow mDataRow in mDtStores.Rows)
                {
                    ...
                    {
                        mCommand.CommandText = "update som_customers set visible_"
                        + mDataRow["code"].ToString().Trim().ToLower() + " = 1 where code='" + mCode.Trim() + "'";
                        mCommand.ExecuteNonQuery();
                    }
                    else
                    {
                        mCommand.CommandText = "update som_customers set visible_"
                        + mDataRow["code"].ToString().Trim().ToLower() + " = 0 where code='" + mCode.Trim() + "'";
                        mCommand.ExecuteNonQuery();
                    }
                }
            ...
            DataTable mDtStores, string mUserId)
        {
            ...
            #region check security for this function
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.ivscustomers_edit.ToString(), mUserId);
            if (mGranted == false)
            {
                mResult.Exe_Result = 0;
                mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.GRL_AccessDeniedToSystemFunction.ToString();
            ...
            try
            {
                mCommand.CommandText = "select * from som_customers where code='" + mCode.Trim() + "'";
                mDataReader = mCommand.ExecuteReader();
                if (mDataReader.Read() == false)
                {
                    mResult.Exe_Result = 0;
                    mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.IVS_CustomerDoesNotExist.ToString();
                    return mResult;
                }
            }
            catch (OdbcException ex)
            ...
            try
            {
                ...
                //som_customers
                mCommand.CommandText = "update som_customers set description = '"
        ...
        public AfyaPro_Types.clsResult Delete(String mCode, string mUserId)
        {
            ...
            #region check security for this function
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.ivscustomers_delete.ToString(), mUserId);
            if (mGranted == false)
            {
                mResult.Exe_Result = 0;
                mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.GRL_AccessDeniedToSystemFunction.ToString();
            ...
            try
            {
                ...
                mCommand.CommandText = "delete from som_customers where code='" + mCode.Trim() + "'";
                mCommand.ExecuteNonQuery();
