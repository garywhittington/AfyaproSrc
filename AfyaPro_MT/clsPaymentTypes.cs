...
{
    public class clsPaymentTypes : MarshalByRefObject
    {
        #region declaration

        private static String pClassName = "AfyaPro_MT.clsPaymentTypes";
        ...
        public DataTable View(String mFilter, String mOrder, string mLanguageName, string mGridName)
        {
            ...
            try
            {
                string mCommandText = "select * from facilitypaymenttypes";
        ...
        public AfyaPro_Types.clsResult Add(Int16 mGenerateCode, String mCode, String mDescription,
            int mPreventOverPay, int mCheckDepositBalance, int mIsCheque, int mAllowRefund, int mSales,
            int mDebtorPayments, string mUserId)
        {
            ...
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.blspaymenttypes_add.ToString(), mUserId);
            if (mGranted == false)
            {
            ...
            if (mGenerateCode == 1)
            {
                ...
                mObjCode = objAutoCodes.Next_Code(
                    Convert.ToInt16(AfyaPro_Types.clsEnums.SystemGeneratedCodes.paymenttypecode), "facilitypaymenttypes", "code");
            ...
            try
            {
                mCommand.CommandText = "select * from facilitypaymenttypes where code='"
                + mCode.Trim() + "'";
                mDataReader = mCommand.ExecuteReader();
                if (mDataReader.Read() == true)
                {
                    mResult.Exe_Result = 0;
                    mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.BLS_PaymentTypeCodeIsInUse.ToString();
                    return mResult;
                }
            }
            ...
            try
            {
                ...
                + mCode.Trim() + "','" + mDescription.Trim() + "'," + mPreventOverPay + "," 
                + mCheckDepositBalance + "," + mIsCheque + "," + mAllowRefund + "," + mSales + "," + mDebtorPayments + ")";
                mCommand.ExecuteNonQuery();

                #region synchronize paymenttypes
                ...
                mAlterTableStr = clsGlobal.Build_AlterTableForPaymentTypes("billcollections", mCode, mDtColumns);
                ...
                mAlterTableStr = clsGlobal.Build_AlterTableForPaymentTypes("billreceipts", mCode, mDtColumns);
                if (mAlterTableStr.Trim() != "")
                {
                    mCommand.CommandText = mAlterTableStr;
                    mCommand.ExecuteNonQuery();
                ...
                mAlterTableStr = clsGlobal.Build_AlterTableForPaymentTypes("billinvoicepayments", mCode, mDtColumns);
                if (mAlterTableStr.Trim() != "")
                {
                    mCommand.CommandText = mAlterTableStr;
                    mCommand.ExecuteNonQuery();
                ...
                string mFieldName = "paytype" + mCode.Trim().ToLower();
                ...
                if (mGenerateCode == 1)
                {
                    ...
                    + Convert.ToInt16(AfyaPro_Types.clsEnums.SystemGeneratedCodes.paymenttypecode);
                    mCommand.ExecuteNonQuery();
                }
            ...
            int mDebtorPayments, string mUserId)
        {
            ...
            #region check security for this function
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.blspaymenttypes_edit.ToString(), mUserId);
            if (mGranted == false)
            {
                mResult.Exe_Result = 0;
                mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.GRL_AccessDeniedToSystemFunction.ToString();
            ...
            try
            {
                mCommand.CommandText = "select * from facilitypaymenttypes where code='" + mCode.Trim() + "'";
                mDataReader = mCommand.ExecuteReader();
                if (mDataReader.Read() == false)
                {
                    mResult.Exe_Result = 0;
                    mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.BLS_PaymentTypeCodeDoesNotExist.ToString();
                    return mResult;
                }
            }
            catch (OdbcException ex)
            ...
            try
            {
                ...
                mCommand.CommandText = "update facilitypaymenttypes set description='"
                + mDescription.Trim() + "',preventoverpay=" + mPreventOverPay + ",checkdepositbalance="
                + mCheckDepositBalance + ",ischeque=" + mIsCheque + ",allowrefund=" + mAllowRefund
                + ",visibilitysales=" + mSales + ",visibilitydebtorpayments=" + mDebtorPayments 
        ...
        public AfyaPro_Types.clsResult Delete(String mCode, string mUserId)
        {
            ...
            #region check security for this function
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.blspaymenttypes_delete.ToString(), mUserId);
            if (mGranted == false)
            {
                mResult.Exe_Result = 0;
                mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.GRL_AccessDeniedToSystemFunction.ToString();
            ...
            try
            {
                ...
                mCommand.CommandText = "delete from facilitypaymenttypes where code='" + mCode.Trim() + "'";
                mCommand.ExecuteNonQuery();
