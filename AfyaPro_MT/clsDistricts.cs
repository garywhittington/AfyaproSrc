...
{
    public class clsDistricts : MarshalByRefObject
    {
        #region declaration

        private static String pClassName = "AfyaPro_MT.clsDistricts";
        ...
        public DataTable View(String mFilter, String mOrder, string mLanguageName, string mGridName)
        {
            ...
            try
            {
                string mCommandText = "select * from districts";
                ...
                foreach (DataRow mDataRow in mDataTable.Rows)
                {
                    ...
                    mRowIndex = mDvRegions.Find(mDataRow["regioncode"].ToString().Trim());
                    if (mRowIndex >= 0)
                    {
                        mRegionDescription = mDvRegions[mRowIndex]["description"].ToString().Trim();
                    }

                    mDataRow["countrydescription"] = mCountryDescription;
                    mDataRow["regiondescription"] = mRegionDescription;
                    mDataRow.EndEdit();
        ...
        public AfyaPro_Types.clsResult Add(Int16 mGenerateCode, String mCode, String mDescription,
            String mCountryCode, String mRegionCode, string mUserId)
        {
            ...
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.gendistricts_add.ToString(), mUserId);
            if (mGranted == false)
            {
            ...
            if (mGenerateCode == 1)
            {
                ...
                mObjCode = objAutoCodes.Next_Code(
                    Convert.ToInt16(AfyaPro_Types.clsEnums.SystemGeneratedCodes.districtcode), "districts", "code");
            ...
            try
            {
                mCommand.CommandText = "select * from districts where code='"
                + mCode.Trim() + "'";
                mDataReader = mCommand.ExecuteReader();
                if (mDataReader.Read() == true)
                {
                    mResult.Exe_Result = 0;
                    mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.GEN_DistrictCodeIsInUse.ToString();
                    return mResult;
                }
            }
            ...
            try
            {
                ...
                //execute statements
                mCommand.CommandText = "insert into districts(code,description,countrycode,regioncode) values('"
                + mCode.Trim() + "','" + mDescription.Trim() + "','" + mCountryCode.Trim() + "','" + mRegionCode.Trim() + "')";
        ...
        public AfyaPro_Types.clsResult Edit(String mCode, String mDescription, string mUserId)
        {
            ...
            #region check security for this function
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.gendistricts_edit.ToString(), mUserId);
            if (mGranted == false)
            {
                mResult.Exe_Result = 0;
                mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.GRL_AccessDeniedToSystemFunction.ToString();
            ...
            try
            {
                mCommand.CommandText = "select * from districts where code='" + mCode.Trim() + "'";
                mDataReader = mCommand.ExecuteReader();
                if (mDataReader.Read() == false)
                {
                    mResult.Exe_Result = 0;
                    mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.GEN_DistrictCodeDoesNotExist.ToString();
                    return mResult;
                }
            }
            catch (OdbcException ex)
            ...
            try
            {
                ...
                mCommand.CommandText = "update districts set description = '"
        ...
        public AfyaPro_Types.clsResult Delete(String mCode, string mUserId)
        {
            ...
            #region check security for this function
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.gendistricts_delete.ToString(), mUserId);
            if (mGranted == false)
            {
                mResult.Exe_Result = 0;
                mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.GRL_AccessDeniedToSystemFunction.ToString();
            ...
            try
            {
                ...
                mCommand.CommandText = "delete from districts where code='" + mCode.Trim() + "'";
                mCommand.ExecuteNonQuery();
