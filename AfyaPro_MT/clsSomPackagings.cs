...
{
    public class clsSomPackagings : MarshalByRefObject
    {
        #region declaration

        private static String pClassName = "AfyaPro_MT.clsSomPackagings";
        ...
        public DataTable View(String mFilter, String mOrder, string mLanguageName, string mGridName)
        {
            ...
            try
            {
                string mCommandText = "select * from som_packagings";
            ...
            int mPieces, string mUserId)
        {
            ...
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.ivspackagings_add.ToString(), mUserId);
            if (mGranted == false)
            {
            ...
            if (mGenerateCode == 1)
            {
                ...
                mObjCode = objAutoCodes.Next_Code(
                    Convert.ToInt16(AfyaPro_Types.clsEnums.SystemGeneratedCodes.packagingcode), "som_packagings", "code");
            ...
            try
            {
                mCommand.CommandText = "select * from som_packagings where code='"
                + mCode.Trim() + "'";
                mDataReader = mCommand.ExecuteReader();
                if (mDataReader.Read() == true)
                {
                    mResult.Exe_Result = 0;
                    mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.IVS_PackagingCodeIsInUse.ToString();
                    return mResult;
                }
            }
            ...
            try
            {
                ...
                //execute statements
                mCommand.CommandText = "insert into som_packagings(code,description,pieces) values('"
                + mCode.Trim() + "','" + mDescription.Trim() + "'," + mPieces + ")";
                ...
                if (mGenerateCode == 1)
                {
                    ...
                    + Convert.ToInt16(AfyaPro_Types.clsEnums.SystemGeneratedCodes.packagingcode);
                    mCommand.ExecuteNonQuery();
                }
        ...
        #region Edit
        public AfyaPro_Types.clsResult Edit(String mCode, String mDescription, int mPieces, string mUserId)
        {
            ...
            #region check security for this function
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.ivspackagings_edit.ToString(), mUserId);
            if (mGranted == false)
            {
                mResult.Exe_Result = 0;
                mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.GRL_AccessDeniedToSystemFunction.ToString();
            ...
            try
            {
                mCommand.CommandText = "select * from som_packagings where code='" + mCode.Trim() + "'";
                mDataReader = mCommand.ExecuteReader();
                if (mDataReader.Read() == false)
                {
                    mResult.Exe_Result = 0;
                    mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.IVS_PackagingDoesNotExist.ToString();
                    return mResult;
                }
            }
            catch (OdbcException ex)
            ...
            try
            {
                ...
                //som_packagings
                mCommand.CommandText = "update som_packagings set description = '"
                + mDescription.Trim() + "',pieces=" + mPieces + " where code='" + mCode.Trim() + "'";
                mCommand.ExecuteNonQuery();

                //som_products
                mCommand.CommandText = "update som_products set packagingdescription = '"
                + mDescription.Trim() + "' where packagingcode='" + mCode.Trim() + "'";
                mCommand.ExecuteNonQuery();
        ...
        public AfyaPro_Types.clsResult Delete(String mCode, string mUserId)
        {
            ...
            #region check security for this function
            bool mGranted = clsGlobal.GrantDeny_FunctionAccess(mCommand,
                AfyaPro_Types.clsSystemFunctions.FunctionKeys.ivspackagings_delete.ToString(), mUserId);
            if (mGranted == false)
            {
                mResult.Exe_Result = 0;
                mResult.Exe_Message = AfyaPro_Types.clsSystemMessages.MessageIds.GRL_AccessDeniedToSystemFunction.ToString();
            ...
            #region check if packaging is in use products
            try
            {
                mCommand.CommandText = "select * from som_products where packagingcode='" + mCode.Trim() + "'";
                mDataReader = mCommand.ExecuteReader();
                if (mDataReader.Read() == true)
                {
                    mResult.Exe_Result = 0;
            ...
            try
            {
                ...
                mCommand.CommandText = "delete from som_packagings where code='" + mCode.Trim() + "'";
                mCommand.ExecuteNonQuery();
