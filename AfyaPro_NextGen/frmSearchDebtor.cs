...
{
    public partial class frmSearchDebtor : DevExpress.XtraEditors.XtraForm
    {
        #region declaration
        ...
        private string pDebtorType = "";
        internal string DebtorType
        {
            get { return pDebtorType; }
            set { pDebtorType = value; }
        }

        #endregion

        #region frmSearchDebtor
        public frmSearchDebtor(object mSourceObject)
        {
            InitializeComponent();

            pType = this.GetType();
            pClassName = pType.FullName;
            string mFunctionName = "frmSearchDebtor";
        ...
        #region frmSearchDebtor_Activated
        private void frmSearchDebtor_Activated(object sender, EventArgs e)
        {
        ...
        {
            string mFunctionName = "frmGENSearchEngine_FormClosing";
            ...
            try
            {
                ...
                Program.Save_GridLayout(grdSearch, "grdSearchDebtor");

                Program.gMdtUserActions.Save_DebtorSearching(
        ...
        void Load_DefaultOptions()
        {
            ...
            try
            {
                foreach (DataRow mDataRow in pDtSearchFields.Rows)
                {
                    if (Program.gDtUserActions.Rows.Count > 0)
                    {
                        if (mDataRow["fieldname"].ToString().Trim().ToLower() ==
                            Program.gDtUserActions.Rows[0]["searchdebtorfieldname"].ToString().Trim().ToLower())
                        {
                ...
                if (Program.gDtUserActions.Rows.Count > 0)
                {
                    radOptions.SelectedIndex = Convert.ToInt16(Program.gDtUserActions.Rows[0]["searchdebtorsoption"]);
                    chkSearchWhileTyping.Checked = Convert.ToBoolean(Program.gDtUserActions.Rows[0]["searchdebtorswhiletyping"]);
                }
                ...
                if (cboSearchField.ItemIndex == -1)
                {
                    ...
                    cboSearchField.ItemIndex = Program.Get_LookupItemIndex(cboSearchField, "fieldname", "accountdescription");
                }
            }
            catch (Exception ex)
            {
        ...
        private void Fill_LookupData()
        {
            ...
            try
            {
                ...
                DataTable mDtSearchFields = pMdtSearchEngine.View_SearchFields("billdebtors", Program.gLanguageName, "grdGENSearchEngine");
        ...
        private void Search()
        {
            ...
            try
            {
                ...
                mRowFilter = mRowFilter + " and debtortype='" + pDebtorType.Trim() + "'";
                pDtSearchResults = pMdtSearchEngine.Search_Debtor(mRowFilter, mFieldName);
                grdSearch.DataSource = pDtSearchResults;

                //restore current document layout
                Program.Restore_GridLayout(grdSearch, "grdSearchDebtor");
        ...
        private void Get_SelectedItem()
        {
            ...
            try
            {
                ...
                string mCode = mDataRow["accountcode"].ToString().Trim();
