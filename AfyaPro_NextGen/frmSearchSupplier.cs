...
{
    public partial class frmSearchSupplier : DevExpress.XtraEditors.XtraForm
    {
        #region declaration
        ...
        #region frmSearchSupplier
        public frmSearchSupplier(object mSourceObject)
        {
            InitializeComponent();

            pType = this.GetType();
            pClassName = pType.FullName;
            string mFunctionName = "frmSearchSupplier";
        ...
        #region frmSearchSupplier_Load
        private void frmSearchSupplier_Load(object sender, EventArgs e)
        {
            string mFunctionName = "frmSearchSupplier_Load";
        ...
        #region frmSearchSupplier_FormClosing
        private void frmSearchSupplier_FormClosing(object sender, FormClosingEventArgs e)
        {
            string mFunctionName = "frmSearchSupplier_FormClosing";
            ...
            try
            {
                ...
                Program.Save_GridLayout(grdSearch, "grdSearchSupplier");

                Program.gMdtUserActions.Save_SupplierSearching(
        ...
        void Load_DefaultOptions()
        {
            ...
            try
            {
                foreach (DataRow mDataRow in pDtSearchFields.Rows)
                {
                    if (Program.gDtUserActions.Rows.Count > 0)
                    {
                        if (mDataRow["fieldname"].ToString().Trim().ToLower() ==
                            Program.gDtUserActions.Rows[0]["searchsupplierfieldname"].ToString().Trim().ToLower())
                        {
                ...
                if (Program.gDtUserActions.Rows.Count > 0)
                {
                    radOptions.SelectedIndex = Convert.ToInt16(Program.gDtUserActions.Rows[0]["searchsuppliersoption"]);
                    chkSearchWhileTyping.Checked = Convert.ToBoolean(Program.gDtUserActions.Rows[0]["searchsupplierswhiletyping"]);
                }
        ...
        private void Fill_LookupData()
        {
            ...
            try
            {
                ...
                DataTable mDtSearchFields = pMdtSearchEngine.View_SearchFields("som_suppliers", Program.gLanguageName, "grdGENSearchEngine");
        ...
        private void Search()
        {
            ...
            try
            {
                ...
                pDtSearchResults = pMdtSomSuppliers.View(mRowFilter, mFieldName, Program.gLanguageName, "grdIVSSourcesOfStock");
                grdSearch.DataSource = pDtSearchResults;

                //restore current document layout
                Program.Restore_GridLayout(grdSearch, "grdSearchSupplier");
