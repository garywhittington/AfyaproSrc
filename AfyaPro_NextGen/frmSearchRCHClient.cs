...
{
    public partial class frmSearchRCHClient : DevExpress.XtraEditors.XtraForm
    {
        #region declaration
        ...
        #region frmSearchRCHClient
        public frmSearchRCHClient(object mSourceObject)
        {
            InitializeComponent();

            pType = this.GetType();
            pClassName = pType.FullName;
            string mFunctionName = "frmSearchRCHClient";
        ...
        #region frmSearchRCHClient_Load
        private void frmSearchRCHClient_Load(object sender, EventArgs e)
        {
            string mFunctionName = "frmSearchRCHClient_Load";
        ...
        #region frmSearchRCHClient_FormClosing
        private void frmSearchRCHClient_FormClosing(object sender, FormClosingEventArgs e)
        {
            string mFunctionName = "frmSearchRCHClient_FormClosing";
            ...
            try
            {
                ...
                Program.Save_GridLayout(grdSearch, "grdSearchRchClient");

                Program.gMdtUserActions.Save_RCHClientSearching(
        ...
        void Load_DefaultOptions()
        {
            ...
            try
            {
                foreach (DataRow mDataRow in pDtSearchFields.Rows)
                {
                    if (Program.gDtUserActions.Rows.Count > 0)
                    {
                        if (mDataRow["fieldname"].ToString().Trim().ToLower() ==
                            Program.gDtUserActions.Rows[0]["searchrchclientfieldname"].ToString().Trim().ToLower())
                        {
                ...
                if (Program.gDtUserActions.Rows.Count > 0)
                {
                    radOptions.SelectedIndex = Convert.ToInt16(Program.gDtUserActions.Rows[0]["searchrchclientsoption"]);
                    chkSearchWhileTyping.Checked = Convert.ToBoolean(Program.gDtUserActions.Rows[0]["searchrchclientswhiletyping"]);
                }
        ...
        private void Fill_LookupData()
        {
            ...
            try
            {
                ...
                DataTable mDtSearchFields = pMdtSearchEngine.View_SearchFields("rch_patients", Program.gLanguageName, "grdGENSearchEngine");
        ...
        private void Search()
        {
            ...
            try
            {
                ...
                {
                    mRowFilter = mRowFilter + " AND fromrch=1";
                }

                DataTable mDtSearchResults = pMdtSearchEngine.Get_RCHPatients(mRowFilter, "");
                grdSearch.DataSource = mDtSearchResults;

                //restore current document layout
                Program.Restore_GridLayout(grdSearch, "grdSearchRchClient");
