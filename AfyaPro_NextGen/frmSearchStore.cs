...
{
    public partial class frmSearchStore : DevExpress.XtraEditors.XtraForm
    {
        #region declaration
        ...
        #region frmSearchStore
        public frmSearchStore(object mSourceObject)
        {
            InitializeComponent();

            pType = this.GetType();
            pClassName = pType.FullName;
            string mFunctionName = "frmSearchStore";
        ...
        #region frmSearchStore_Load
        private void frmSearchStore_Load(object sender, EventArgs e)
        {
            string mFunctionName = "frmSearchStore_Load";
        ...
        #region frmSearchStore_FormClosing
        private void frmSearchStore_FormClosing(object sender, FormClosingEventArgs e)
        {
            string mFunctionName = "frmSearchStore_FormClosing";
            ...
            try
            {
                ...
                Program.Save_GridLayout(grdSearch, "grdSearchStore");

                Program.gMdtUserActions.Save_StoreSearching(
        ...
        private void Grid_Settings()
        {
            ...
            try
            {
                ...
                mGridView1.OptionsNavigation.AutoMoveRowFocus = false;
                mGridView1.OptionsNavigation.AutoFocusNewRow = false;
        ...
        void Load_DefaultOptions()
        {
            ...
            try
            {
                foreach (DataRow mDataRow in pDtSearchFields.Rows)
                {
                    if (Program.gDtUserActions.Rows.Count > 0)
                    {
                        if (mDataRow["fieldname"].ToString().Trim().ToLower() ==
                            Program.gDtUserActions.Rows[0]["searchstorefieldname"].ToString().Trim().ToLower())
                        {
                ...
                if (Program.gDtUserActions.Rows.Count > 0)
                {
                    radOptions.SelectedIndex = Convert.ToInt16(Program.gDtUserActions.Rows[0]["searchstoresoption"]);
                    chkSearchWhileTyping.Checked = Convert.ToBoolean(Program.gDtUserActions.Rows[0]["searchstoreswhiletyping"]);
                }
        ...
        private void Fill_LookupData()
        {
            ...
            try
            {
                ...
                DataTable mDtSearchFields = pMdtSearchEngine.View_SearchFields("som_stores", Program.gLanguageName, "grdGENSearchEngine");
        ...
        private void Search()
        {
            ...
            try
            {
                ...
                string mStores = "";
                foreach (DataRow mDataRow in Program.gDtUserStores.Rows)
                {
                    if (mStores.Trim() == "")
                    {
                        mStores = "'" + mDataRow["storecode"].ToString().Trim() + "'";
                    }
                    else
                    {
                        mStores = mStores + ",'" + mDataRow["storecode"].ToString().Trim() + "'";
                    }
                }
                ...
                {
                    mStores = "code in (" + mStores + ")";
                }

                mRowFilter = mRowFilter + " and " + mStores;

                pDtSearchResults = pMdtSomStores.View(mRowFilter, mFieldName, Program.gLanguageName, "grdIVSStores");
                grdSearch.DataSource = pDtSearchResults;

                //restore current document layout
                Program.Restore_GridLayout(grdSearch, "grdSearchStore");
