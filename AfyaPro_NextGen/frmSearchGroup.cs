...
{
    public partial class frmSearchGroup : DevExpress.XtraEditors.XtraForm
    {
        #region declaration
        ...
        #region frmSearchGroup
        public frmSearchGroup(object mSourceObject)
        {
            InitializeComponent();

            pType = this.GetType();
            pClassName = pType.FullName;
            string mFunctionName = "frmSearchGroup";
        ...
        #region frmSearchGroup_Load
        private void frmSearchGroup_Load(object sender, EventArgs e)
        {
            string mFunctionName = "frmSearchGroup_Load";
        ...
        #region frmSearchGroup_Activated
        private void frmSearchGroup_Activated(object sender, EventArgs e)
        {
        ...
        #region frmSearchGroup_FormClosing
        private void frmSearchGroup_FormClosing(object sender, FormClosingEventArgs e)
        {
            string mFunctionName = "frmSearchGroup_FormClosing";
            ...
            try
            {
                ...
                Program.Save_GridLayout(grdSearch, "grdSearchGroup");

                Program.gMdtUserActions.Save_ClientGroupSearching(
        ...
        void Load_DefaultOptions()
        {
            ...
            try
            {
                foreach (DataRow mDataRow in pDtSearchFields.Rows)
                {
                    if (Program.gDtUserActions.Rows.Count > 0)
                    {
                        if (mDataRow["fieldname"].ToString().Trim().ToLower() ==
                            Program.gDtUserActions.Rows[0]["searchclientgroupfieldname"].ToString().Trim().ToLower())
                        {
                ...
                if (Program.gDtUserActions.Rows.Count > 0)
                {
                    radOptions.SelectedIndex = Convert.ToInt16(Program.gDtUserActions.Rows[0]["searchclientgroupsoption"]);
                    chkSearchWhileTyping.Checked = Convert.ToBoolean(Program.gDtUserActions.Rows[0]["searchclientgroupswhiletyping"]);
                }
        ...
        private void Fill_LookupData()
        {
            ...
            try
            {
                ...
                DataTable mDtSearchFields = pMdtSearchEngine.View_SearchFields("facilitycorporates", Program.gLanguageName, "grdGENSearchEngine");
        ...
        private void Search()
        {
            ...
            try
            {
                ...
                pDtSearchResults = pMdtClientGroups.View(mRowFilter, mFieldName, Program.gLanguageName, "grdSearchGroup");
                grdSearch.DataSource = pDtSearchResults;

                //restore current document layout
                Program.Restore_GridLayout(grdSearch, "grdSearchGroup");
